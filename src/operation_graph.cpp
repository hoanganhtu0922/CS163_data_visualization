...
void operation_graph::initialize(int _x, int _y, bool _isabc, bool is_dijkstra) {
    ...
    y = _y;
    isabc = _isabc;
    options = {"Intit", "Vertices", "Edges", "Kruskal"};
    if (is_dijkstra) {
        options = {"Intit", "Vertices", "Edges", "start", "Dijkstra"};
    }
}
...
void operation_graph::update() {
    ...
    for (auto str : options) {
        ...
        if (!input.focused && CheckCollisionPointRec(GetMousePosition(), textRect) && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
            command = str;
            if (command == options.back() || command == options[0]) {
                is_pending = true;
                return;
            }
            input.appear = true;
            input.initialize(x + 250, y + 20 + id * 40, 200, 40, "Enter value");
            break;
        }
        id++;
    }
